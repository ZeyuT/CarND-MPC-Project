 ...
 public:
  ...
  void operator()(ADvector& fg, const ADvector& vars) {
    ...
    for (unsigned int i = 0; i < N - 1; i++) {
      fg[0] += 0.1 * CppAD::pow(vars[delta_start + i], 2);
	...
	//Initializ the model to the initial state
	...
	for (unsigned int t = 1; t < N; t++) {
		...
		AD<double> a;
		AD<double> delta;
		// Consider latency (0.1s). Use a and delta of the previous step.
		if (t > 1) {
			a = vars[a_start + t - 2];
		...
		// The reference y and orientation at time t.
